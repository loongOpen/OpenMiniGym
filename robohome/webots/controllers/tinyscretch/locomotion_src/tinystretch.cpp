...
void* Thread_UDP_OCU(void*)//OCU UDP通讯线程  as 服务器作为
{
    static int cnt = 0;
    float sys_dt = 0;
        ...
        if(recv_num >0)
        {
            ...
            if(recv_num==12){//recording
                ...
                if(recording_flag&&(_remote_msg_record.record_rate>0.01||robotwb.start_record==0))
                {
                    ...
                    if(_remote_msg_record.record_rate>0.99||robotwb.start_record==0){
                        recording_flag=0;
...
int main(int argc, char **argv) {
  ...
  pthread_create(&tida, NULL, Thread_UDP_OCU, NULL);
