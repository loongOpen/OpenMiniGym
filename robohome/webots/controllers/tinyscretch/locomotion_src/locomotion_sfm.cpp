...
void reset_tar_pos(char sel,char init_q) {//区分着地离地？<<----------------------------!!!!!!!!!!!!!!!!
    ...
    {
        if(init_q)
          robotwb.arm_q_exp_flt[sel][i]=robotwb.arm_q_exp[sel][i]=robotwb.arm_q[sel][i];
        else
          robotwb.arm_q_exp[sel][i]=robotwb.arm_q[sel][i];
    }
    if(init_q)
    ...
    robotwb.arm_att_h_exp=robotwb.arm_att_h;
...
void locomotion_sfm(float dt)
{
   ...
   case 1://------------
       ...
       if(timer[0]>0.01){
            ...
            temp_flag[0]+=move_joint_to_pos_sel1(&robotwb,99,robotwb.safe_height,100*gain_all,0.01,0,dt);//height
            temp_flag[0]+=move_joint_to_pos_sel1(&robotwb,0,robotwb.safe_arm_q[0][0],250*gain_all,2,0,dt);//d
            temp_flag[0]+=move_joint_to_pos_sel1(&robotwb,1,robotwb.safe_arm_q[0][1],500*gain_all,2,0,dt);//x
            temp_flag[0]+=move_joint_to_pos_sel1(&robotwb,2,robotwb.safe_arm_q[0][2],250*gain_all,2,0,dt);//doghome
            temp_flag[0]+=move_joint_to_pos_sel1(&robotwb,3,robotwb.safe_arm_q[0][3],250*gain_all,2,0,dt);//doghome
            temp_flag[0]+=move_joint_to_pos_sel1(&robotwb,4,robotwb.safe_arm_q[0][4],250*gain_all,2,0,dt);//doghome
       }

       if(temp_flag[0]==6){//可强制结束 Lock now angle
           printf("Motor Initing Done!!\n");
           robotwb.state_gait++;
           robotwb.cmd_robot_state=1;
           reset_tar_pos(0,0);
           vmc_reset();
        }
    break;
...
char gait_switch(float dt)//cushihua  moshi  主步态切换状态机
{
    ...
    if ((((ocu.connect&&ocu.mode >= 2) || sdk.sdk_mode == 1) && !ocu.sbus_conncect) || (RUN_WEBOTS)) {//锟斤拷通锟街憋拷
