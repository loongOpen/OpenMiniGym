...
void traj_planner(float dt)
{
    ...
    //base
    DigitalLPF(robotwb.base_vel_b_exp_rc.x, &robotwb.base_vel_b_exp_rc_flt.x, 0.5, dt);
    DigitalLPF(robotwb.base_rate_exp_rc, &robotwb.base_rate_exp_rc_flt, 0.7, dt);
    ...
    if(robotwb.robot_mode==STAND_RC){
        ...
        case 0:
            if(robotwb.arm_control_mode==ARM_M_E&&robotwb.arm_control_mode!=arm_control_mode_reg){
                ...
                robotwb.arm_epos_b_exp.z=robotwb.arm_epos_h.z;
...
#if RUN_WEBOTS
         float d_w=robotwb.base_rate_exp/(robotwb.wheel_w/2)*1.2;
#else
         float d_w=robotwb.base_rate_exp/(robotwb.wheel_w/2);
#endif
         int cnt_l=robotwb.wheel_touch[0]+robotwb.wheel_touch[2];
         int cnt_r=robotwb.wheel_touch[1]+robotwb.wheel_touch[3];
         if(cnt_l>0&&cnt_r==0)
         {
             wheel_dq_exp[0]-=d_w*robotwb.wheel_touch[0];
             wheel_dq_exp[2]-=d_w*robotwb.wheel_touch[2];
         }else if(cnt_r>0&&cnt_l==0)
         {
             wheel_dq_exp[1]-=d_w*robotwb.wheel_touch[1];
             wheel_dq_exp[3]-=d_w*robotwb.wheel_touch[3];
         }else{
             wheel_dq_exp[0]-=d_w;
             wheel_dq_exp[1]+=d_w;
             wheel_dq_exp[2]-=d_w;
