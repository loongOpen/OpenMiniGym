...
typedef struct
{
    ...
    PIDs f_pos_pid_st[5], f_pos_pid_sw[5];
    PIDs f_pid_st[5], f_pid_sw[5];
    PIDs q_pid, f_pid[5], f_pos_pid[5];
...
typedef struct
{
    ...
    Vect3 vect3_zero;
    eulerAngleTypeDef   IMU_now, IMU_now_o, IMU_now_e_off;        //机器人当前欧拉角
    ...
    float cap_set,cap_set_flt;
    float cap_set_sdk[2];

    float arm_base_height_exp;
    float arm_base_height_exp_flt;
    float arm_base_height;
    float arm_base_dheight;
...
typedef struct
{
    ...
    float bat_v[2];
    float temp[2];
    float q_now[6],qd_now[6],qdd_now[6];
