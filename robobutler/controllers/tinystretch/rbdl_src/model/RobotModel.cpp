...
RobotModel::RobotModel(){
  ...
  if (!Addons::URDFReadFromFile (
              THIS_COM, model_, true, false)) {
    std::cerr << THIS_COM<< "Error loading model.urdf" << std::endl;
    abort();
  }
...
void RobotModel::getAg_AgdotQdot(MatrixXd & Ag, MatrixXd & AgdotQdot){
  ...
  MatrixXd joint_lin_mat = H.block(0,6,3,NUM_DOF);
  MatrixXd joint_ang_mat = H.block(3,6,3,NUM_DOF);
  //moemntum: rpy xyz
  //vel: rpy xyz joint
  ...
  H.block(0,6,3,NUM_DOF) = joint_ang_mat;
  H.block(3,6,3,NUM_DOF) = joint_lin_mat;
  ...
  Ag.block(0,0,3,NUM_DQ) = (base_X_com.toMatrixTranspose() * body_X_base.toMatrixTranspose() * U1 * H).block(0,0,3,NUM_DQ);
