...
int mem_connect_sim=0;
float mem_loss_cnt_sim=0;
...
static struct shareMemory shareMemory_spi,shareMemory_control;

static unsigned char mem_read_buf[MEM_SIZE];
static unsigned char mem_write_buf[MEM_SIZE];
static long mem_read_cnt=0;
static long mem_write_cnt=0;
...
static void memory_write(void)//写入Memeory to ROS
{
...
void* Thread_Mem_Webots(void*)//内存管理线程
{
    ...
    while(1)
    {
       //共享内存读取 to Servo task
        ...
        mem_loss_cnt_sim+=sys_dt;
        if(mem_loss_cnt_sim>1&&mem_connect_sim==1){
            mem_connect_sim=0;
            mem_loss_cnt_sim=0;
            mem_init_cnt=0;
