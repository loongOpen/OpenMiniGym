...
void cal_ero_so3(float exp_att[3], float *err_so3) {
	...
	exp_angle[PITr] = exp_att[PITr];
	exp_angle[ROLr] = exp_att[ROLr];
	exp_angle[YAWr] = exp_att[YAWr];
	...
	R_control_set[0][0] = cosd(-exp_angle[PITr])*cosd(-exp_angle[YAWr]);
	R_control_set[1][0] = -cosd(-exp_angle[ROLr])*sind(-exp_angle[YAWr]) + sind(-exp_angle[PITr])*sind(-exp_angle[ROLr])*cosd(-exp_angle[YAWr]);
	R_control_set[2][0] = sind(-exp_angle[ROLr])*sind(-exp_angle[YAWr]) + cosd(-exp_angle[ROLr])*sind(-exp_angle[PITr])*cosd(-exp_angle[YAWr]);

	R_control_set[0][1] = cosd(-exp_angle[PITr])*sind(-exp_angle[YAWr]);
	R_control_set[1][1] = cosd(-exp_angle[ROLr])*cosd(-exp_angle[YAWr]) + sind(-exp_angle[ROLr])*sind(-exp_angle[PITr])*sind(-exp_angle[YAWr]);
	R_control_set[2][1] = -sind(-exp_angle[ROLr])*cosd(-exp_angle[YAWr]) + cosd(-exp_angle[ROLr])*sind(-exp_angle[PITr])*sind(-exp_angle[YAWr]);

	R_control_set[0][2] = -sind(-exp_angle[PITr]);
	R_control_set[1][2] = sind(-exp_angle[ROLr])*cosd(-exp_angle[PITr]);
	R_control_set[2][2] = cosd(-exp_angle[ROLr])*cosd(-exp_angle[PITr]);
...
e_R(2) = atan2f((R_rp_x % R_sp_x) * R_sp_z, R_rp_x * R_sp_x) * yaw_w; */
	...
	*(err_so3 + 0) =  e_R[1] * 57.3;//x
	*(err_so3 + 1) =  e_R[0] * 57.3;//y
	*(err_so3 + 2) =  e_R[2] * 57.3;//z
}
