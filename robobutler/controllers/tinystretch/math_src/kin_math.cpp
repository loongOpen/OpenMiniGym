...
// 检查点是否在椭圆内部或边界上
bool isInEllipse(const Point& point, double a, double b) {
    return (point.x * point.x) / (a * a) + (point.y * point.y) / (b * b) <= 1.0;
}

// 调整点到两个椭圆之间
Point adjustPointToBetweenEllipses(const Point& point, double aOuter, double bOuter, double aInner, double bInner) {
    Point adjusted = point;
    double scaleOuter = std::sqrt((adjusted.x * adjusted.x) / (aOuter * aOuter) + (adjusted.y * adjusted.y) / (bOuter * bOuter));
    double scaleInner = std::sqrt((adjusted.x * adjusted.x) / (aInner * aInner) + (adjusted.y * adjusted.y) / (bInner * bInner));

    // 如果点在外部椭圆外，按比例缩小坐标值
    if (scaleOuter > 1) {
        adjusted.x /= scaleOuter;
        adjusted.y /= scaleOuter;
    }
    // 如果点在内部椭圆内，按比例增大坐标值
    else if (scaleInner < 1) {
        // 需要特别处理完全在椭圆中心的情况，以避免除以0的错误
        if (adjusted.x == 0 && adjusted.y == 0) {
            adjusted.x = aInner; // 随便设置一个在内椭圆边界上的点
        } else {
            adjusted.x *= 1.1 / scaleInner; // 这里的放大系数1.1是为了确保点落在内椭圆外
            adjusted.y *= 1.1 / scaleInner;
        }
    }
...
void ik_hip_protect(END_POS* eepos)
{
    float line_y=0.12;
    float line_x=0.2;
    double aOuter = 0.6; // 外部椭圆的长轴
    double bOuter = 0.45; // 外部椭圆的短轴

    double aInner = 0.30; // 内部椭圆的长轴
    double bInner = 0.14; // 内部椭圆的短轴
    ...
    if(robotwb.arm_epos_h_exp.x>0.6)
        robotwb.arm_epos_h_exp.x=0.6;
    if(robotwb.arm_epos_h_exp.y>0.1)
        robotwb.arm_epos_h_exp.y=0.1;
   // printf("limit=%f %f\n",robotwb.arm_epos_h_exp.x,robotwb.arm_epos_h_exp.y);
    ...
    // 首先检查点是否已经在两个椭圆之间
    //std::cout << "origin Point: (" << p.x << ", " << p.y << ")" << std::endl;
    if (!isInEllipse(p, aOuter, bOuter) || isInEllipse(p, aInner, bInner)) {
       p = adjustPointToBetweenEllipses(p, aOuter, bOuter, aInner, bInner);
       //std::cout << "Adjusted Point: (" << p.x << ", " << p.y << ")" << std::endl;
       eepos->x=p.x;
       eepos->y=p.y;
    } else {
       //std::cout << "Point is already between the ellipses: (" << p.x << ", " << p.y << ")" << std::endl;
    }

    robotwb.arm_base_height_exp=LIMIT(robotwb.arm_base_height_exp,robotwb.min_epos_z,robotwb.max_epos_z);
}
...
#else
    fk_rbdl(q_in, &end_pos_now, &end_att_now);
#endif

#if 0
    static int cnt_p=0;
    if(cnt_p++>50){cnt_p=0;
        printf("rbl:x=%.3f y=%.3f z=%.3f\n",end_pos_now.x,end_pos_now.y,end_pos_now.z);
        printf("sen:z=%.3f q=%.3f %.3f %.3f %.3f %.3f\n",q_in[0],q_in[1]*57.3,q_in[2]*57.3,q_in[3],q_in[4]*57.3,q_in[5]*57.3);
