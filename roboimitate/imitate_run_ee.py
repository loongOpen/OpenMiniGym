...
def main(args):
    ...
    if policy_class == 'ACT':
        ...
        policy_config = {'lr': args['lr'],
                         ...
                         'vq_dim': args['vq_dim'],
                         'action_dim': (6+DOF_CAP)*2+2, #16,  #amao 16  14arm + base_vel 2
...
def eval_bc(config, ckpt_name, save_episode=True, num_rollouts=50):
    ...
    for rollout_id in range(num_rollouts):
        ...
        if real_robot:
            ...
            np.save(os.path.join(ckpt_dir, f'epos_{log_id}.npy'), qpos_history_raw)
            ...
            for i in range(state_dim):
                ...
                if i != state_dim - 1:
                    plt.xticks([])
            plt.tight_layout()
            plt.savefig(os.path.join(ckpt_dir, f'epos_{log_id}.png'))
            plt.close()
