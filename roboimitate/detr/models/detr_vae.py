...
class DETRVAE(nn.Module):
    ...
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names, vq, vq_class, vq_dim, action_dim):
        ...
        if backbones is not None:
            ...
            self.input_proj_robot_state = nn.Linear((DOF_ARM+DOF_CAP)*2, hidden_dim)
        else:
            # input_dim = 14 + 7 # robot_state + env_state
            self.input_proj_robot_state = nn.Linear((DOF_ARM+DOF_CAP)*2, hidden_dim)#
            self.input_proj_env_state = nn.Linear(7, hidden_dim)#doghome   cube pose in simulation
        ...
        #self.encoder_joint_proj = nn.Linear(state_dim, hidden_dim)  # project qpos to embedding doghome
        self.encoder_joint_proj = nn.Linear((DOF_ARM+DOF_CAP)*2, hidden_dim)  # project qpos to embedding
    ...
    def encode(self, qpos, actions=None, is_pad=None, vq_sample=None):
        ...
        if self.encoder is None:
            ...
            if is_training:
                ...
                action_embed = self.encoder_action_proj(actions) # (bs, seq, hidden_dim)  action 14 
                #print("s",len(qpos))
                qpos_embed = self.encoder_joint_proj(qpos)  # (bs, hidden_dim)  qpos 14
...
def build(args):
    state_dim = 16 # TODO hardcode #amao 14
...
def build_cnnmlp(args):
    state_dim = (DOF_ARM+DOF_CAP)*2, # TODO hardcode
