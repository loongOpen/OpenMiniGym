...
class EpisodicDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_path_list, camera_names, norm_stats, episode_ids, episode_len, chunk_size, policy_class):
        ...
        print("EpisodicDataset",camera_names)
        ...
        try:
            ...
            with h5py.File(dataset_path, 'r') as root:
                try: # some legacy data does not have this attribute
                    ...
                    #print("is_sim = root.attrs['sim']",is_sim)
                except:
                    is_sim = False
                compressed = root.attrs.get('compress', False)#-------doghome
                if '/base_action' in root:
                    base_action = root['/base_action'][()]#2
                ...
                qvel = root['/observations/qvel'][start_ts]#14
                image_dict = dict()
                for cam_name in self.camera_names:
                    image_dict[cam_name] = root[f'/observations/images/{cam_name}'][start_ts]
                    #print(cam_name,compressed)
...
def get_norm_stats(dataset_path_list):
    ...
    return stats, all_episode_len # 求解z参数
...
def load_data(dataset_dir_l, name_filter, camera_names, batch_size_train, batch_size_val, chunk_size, skip_mirrored_data=False, load_pretrain=False, policy_class=None, stats_dir_l=None, sample_weights=None, train_ratio=0.99):
    ...
    # construct dataset and dataloader doghome 组织训练数据
    ...
    train_num_workers = 8#(8 if os.getlogin() == 'zfu' else 16) if train_dataset.augment_images else 2
...
def amao_sample_box_pose(cube_position):
    #在初始位置附近，随机初值
    en_random=1
 
    x_range = [-0.035, 0.035]#前向位置不能太far -0.1
    y_range = [-0.07, 0.07]
    z_range = [0.0, 0.0]

    ranges = np.vstack([x_range, y_range, z_range])
    rand_position = np.random.uniform(ranges[:, 0], ranges[:, 1])
    cube_position = cube_position + rand_position*en_random
