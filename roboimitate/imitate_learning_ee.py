...
def main(args):
    ...
    # fixed parameters
    state_dim = (6+DOF_CAP)*2#16 #14 amao  14->arm  nocontain base vel to  
    lr_backbone = 1e-5
    backbone = 'resnet18'
    if policy_class == 'ACT':
        enc_layers = 4#4
        ...
        policy_config = {'lr': args['lr'],
                         ...
                         'vq_dim': args['vq_dim'],
                         'action_dim': (6+DOF_CAP)*2+2,  #amao 16  14arm + base_vel 2
...
def eval_bc(config, ckpt_name, save_episode=True, num_rollouts=50):
    ...
    for rollout_id in range(num_rollouts):
        ...
        elif 'sim_insertion' in task_name: #14,
            BOX_POSE[0] = np.concatenate(sample_insertion_pose()) # used in sim reset
        elif 'human_test' in task_name: #amao
            cube_pose = amao_sample_box_pose([0.28,0.185,0.06]) #red
            cube_pose1 = amao_sample_box_pose([0.28,-0.185,0.06]) #blue
            BOX_POSE[0] = np.concatenate([cube_pose,cube_pose1])
        ...
        if onscreen_render: #amao
            ...
            plt_img1 = ax1.imshow(ts.observation['images']['top'])
        ...
        with torch.inference_mode():
            ...
            for t in range(max_timesteps):
                ...
                if onscreen_render: #amao
                    plt_img1.set_data(ts.observation['images']['top'])
...
if __name__ == '__main__':
    from constants_real import TRAIN_STEP,RETRAIN,DATA_DIR_RUN,DATA_DIR,TASK_NAME
    lens_all=TRAIN_STEP
    parser = argparse.ArgumentParser()
    parser.add_argument('--eval', default=False)
    parser.add_argument('--onscreen_render',default=False)
    parser.add_argument('--ckpt_dir', action='store', type=str, default=DATA_DIR)#'../saved_ckpt') doghome
    parser.add_argument('--policy_class', action='store', type=str, default='ACT')
    parser.add_argument('--task_name', action='store', type=str, default=TASK_NAME)#doghome
    parser.add_argument('--batch_size', action='store', type=int, default=8)
    parser.add_argument('--seed', action='store', type=int, default=0)
    parser.add_argument('--num_steps', action='store', type=int, default=lens_all)#训练迭代次数
    parser.add_argument('--lr', action='store', type=float, default=1e-5)
    if RETRAIN:
        parser.add_argument('--load_pretrain', action='store_true', default=DATA_DIR_RUN)#'../saved_ckpt/policy_step_500_seed_0.ckpt')#repeat
    else:
        parser.add_argument('--load_pretrain', action='store_true', default=None)#first learning

    parser.add_argument('--eval_every', action='store', type=int, default=500, help='eval_every', required=False)
    parser.add_argument('--validate_every', action='store', type=int, default=500, help='validate_every', required=False)
    parser.add_argument('--save_every', action='store', type=int, default=lens_all, help='save_every', required=False)#保存网络的次数
    parser.add_argument('--resume_ckpt_path', default=None) #'../saved_ckpt/policy_best.ckpt'
